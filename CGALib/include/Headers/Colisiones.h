...
//¿cuales son los pasos que hay que hacer para detectar la colicion de un rayo con una caja oriental?
bool testRayOBB(glm::vec3 o, glm::vec3 t, AbstractModel::OBB obb){ //(origen, final, obb)
	glm::quat qinv = glm::inverse(obb.u); //primero creamos un quaternion
	glm::vec3 oTmp = qinv * o; //Transformacion inversa del origen
	glm::vec3 tTmp = qinv * t; //Transformacion inversa del final
	glm::vec3 cAABB = qinv * obb.c;
	glm::vec3 mins = cAABB - obb.e; //calcular los minimos y los maximos
	glm::vec3 maxs = cAABB + obb.e;
	AbstractModel::AABB aabb(mins, maxs);
	return intersectSegmentAABB(oTmp, tTmp, aabb); //aplicar el algoritnmo aabb
}
